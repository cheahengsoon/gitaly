...
// ntpURL is a URL of the NTP service to query, if not set the default pool.ntp.org is used.
// driftThreshold is a time duration that is considered acceptable time offset.
func CheckClockSync(ntpURL string, driftThreshold time.Duration) (bool, error) {
	if ntpURL == "" {
		ntpURL = "pool.ntp.org"
	}

	resp, err := ntp.Query(ntpURL)
	if err != nil {
