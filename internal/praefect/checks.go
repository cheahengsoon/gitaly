...
func NewClockSyncCheck(clockDriftCheck func(ntpURL string, driftThreshold time.Duration) (bool, error)) func(_ config.Config, _ io.Writer, _ bool) *Check {
	return func(conf config.Config, w io.Writer, quite bool) *Check {
		return &Check{
			...
			Run: func(ctx context.Context) error {
				...
				const driftThresholdMillisEnvName = "DRIFT_THRESHOLD_MILLIS"
				if os.Getenv(driftThresholdMillisEnvName) != "" {
					driftThresholdMillis, err := strconv.Atoi(os.Getenv(driftThresholdMillisEnvName))
					if err != nil {
						return fmt.Errorf("env var %s expected to be an integer", driftThresholdMillisEnvName)
					}
					driftThreshold = time.Duration(int(time.Millisecond) * driftThresholdMillis)
				}

				ntpURL := "pool.ntp.org"
				if os.Getenv("NTP_URL") != "" {
					ntpURL = os.Getenv("NTP_URL")
				}

				correlationID := correlation.SafeRandomID()
				ctx = correlation.ContextWithCorrelation(ctx, correlationID)

				logMessage(quite, w, "checking with NTP service at %s and allowed clock drift %d ms [correlation_id: %s]", ntpURL, driftThreshold.Milliseconds(), correlationID)

				g, ctx := errgroup.WithContext(ctx)
				g.Go(func() error {
					synced, err := clockDriftCheck(ntpURL, driftThreshold)
					if err != nil {
					...
					for j := range conf.VirtualStorages[i].Nodes {
						...
						g.Go(func() error {
							...
							resp, err := serverServiceClient.ClockSynced(ctx, &gitalypb.ClockSyncedRequest{
								NtpUrl: ntpURL, DriftThresholdMillis: driftThreshold.Milliseconds(),
							})
							if err != nil {
